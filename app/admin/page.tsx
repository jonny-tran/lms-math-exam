...
export default function AdminPage() {
  ...
  const isAdmin = useMemo(
    () => normalizeRole(profile?.role) === ADMIN_ROLE,
    [profile?.role]
  );
    ...
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-6 px-4 py-10">
      <header>
        ...
        <p className="text-muted-foreground">
          Chỉ admin mới truy cập được khu vực này. Dữ liệu được lấy từ
      ...
      {profile && !isAdmin && (
        <div className="rounded border border-amber-200 bg-amber-50 p-4 text-amber-800">
          <p>
            Tài khoản hiện tại không có quyền Admin (role:{" "}
            <strong>{profile.role}</strong>). Vui lòng sử dụng tài khoản admin.
          </p>
        </div>
      )}
    </main>
